...
import { fixSpawnHelperPermissions, resolveAgentCommand, spawnAgentProcess } from '../src/pty.js'
...
describe('resolveAgentCommand', () => {
  it('returns an absolute existing path unchanged', () => {
    ...
    expect(resolveAgentCommand(self)).toBe(self)
  })

  it('throws a clear error for a missing command', () => {
    expect(() => resolveAgentCommand('openmicro-definitely-missing-xyz')).toThrow(
      /not found on PATH/,
    )
  })
})
