...
describe('dispatchAction', () => {
  ...
  it('clears the composer then sends /clear for new_chat (staged PTY writes)', () => {
    const { deps, writes } = makeDeps()
    dispatchAction({ type: 'new_chat' }, deps)
    expect(writes).toEqual(['\x01\x0b\x15', '/clear\r'])
  })

  it('clears then sends /model for open_model', () => {
    const { deps, writes } = makeDeps()
    dispatchAction({ type: 'open_model' }, deps)
  ...
  it('clears then sends /new for Codex new_chat', () => {
    const { deps, writes } = makeDeps({ harness: codexHarness })
    dispatchAction({ type: 'new_chat' }, deps)
    expect(writes).toEqual(['\x01\x0b\x15', '/new\r'])
  })
})
