...
function run(command: string, args: string[], cwd = root, env = process.env): string {
  // Windows: npm is a batch shim (needs shell). Quote the executable and any
  // args that contain spaces so cmd.exe does not split `C:\repos\free wili 2\...`.
  const useShell = process.platform === 'win32'
  const quote = (a: string): string => (/\s/.test(a) ? `"${a.replace(/"/g, '\\"')}"` : a)
  const cmd = useShell ? quote(command) : command
...
describe('packed controller API', () => {
  ...
  afterAll(() => {
    if (temp) fs.rmSync(temp, { force: true, recursive: true })
  })
