...
function defaultConfigPath(): string {
  // Prefer explicit HOME (tests / unix) so Windows can override without
  // fighting USERPROFILE-backed os.homedir().
  const home = process.env.OPENMICRO_HOME ?? process.env.HOME ?? os.homedir()
  return path.join(home, '.openmicro', 'config.json')
}
